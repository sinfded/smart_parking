    ...
    >
      ...
      <div class="flex items-start justify-between px-5 pt-2 pb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-muted-foreground">{{ categoryLabel[slot.category] ?? slot.category }}</p>
          </div>
        </div>
        <button
      ...
      <div class="px-5 pb-6 space-y-3">
        <div v-if="distanceM != null" class="flex justify-between text-sm">
          ...
          <span class="font-medium">~{{ distanceM }} m</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-muted-foreground">Type</span>
          <span class="font-medium">{{ categoryLabel[slot.category] ?? slot.category }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-muted-foreground">Dimensions</span>
          <span class="font-medium tabular-nums">{{ Math.round(slot.width / 100) / 10 }} × {{ Math.round(slot.height / 100) / 10 }} m</span>
        </div>
...
.slide-up-enter-active { transition: transform 0.25s cubic-bezier(0.32, 0.72, 0, 1); }
